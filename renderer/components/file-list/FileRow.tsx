...
export function FileRow({
	...
	const isResidual = file.status === FileProcessingStatus.Residual;
	const isExpandable = isComplete || isError || isResidual;

	// Honest warning text for the residual state. PDFs get a specific hint since
	// their residue is recoverable-but-hidden and needs a structural rewrite.
	const residualMessage = isResidual
		? file.extension.toLowerCase() === ".pdf"
			? t("residualPdfWarning")
			: t("residualWarning")
		: undefined;
	...
	const rowClasses = [
		...
		isResidual ? "file-table__row--residual" : "",
	...
	return (
		...
		>
			...
			{isExpanded &&
				(file.status === FileProcessingStatus.Complete ||
					file.status === FileProcessingStatus.Residual) &&
...
function renderAfterCell(
	...
	switch (file.status) {
		...
		case FileProcessingStatus.Residual:
			return (
				<span className="file-table__after-done file-table__after-residual">
					{file.afterTags ?? ""}
					<StatusIcon status={file.status} shouldAnimate={false} />
				</span>
			);
